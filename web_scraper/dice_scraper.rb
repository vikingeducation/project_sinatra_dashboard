...
class DiceScraper
  ...
  def initialize(search_subject, zip_code)
    ...
    @results_page = get_results_page(search_subject, zip_code)
    @saver = ScraperSaver.new
  end

  def get_results_page(search_subject, zip_code)
    ...
    # Fill in the field named "l" (location field)
    form_by_action.l = zip_code
  ...
  def get_results_array
    ...
    @results_page.links_with(:href => /p\&q=/).first(10).each do |link|
        job = get_current_job_details(link)
        jobs << [job.title, job.company[0..-2], job.link, job.location, job.posting_date, job.company_id, job.job_id]
    end
    jobs
  end
  ...
  def scrape_and_save(file_name, mode)
    # Going through each job on the results page
    @results_page.links_with(:href => /p\&q=/).limit(10).each do |link|
      # Saving our file through the ScraperSaver class
